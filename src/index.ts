...
export default {
	async fetch(request: Request, env: Env) {
		...
		if (pathname.startsWith('/login')) {
			const state = Math.random().toString(36).substring(7); // TODO: use properly secure random state if necessary
			await env.spotifyState.put('state', state, { expirationTtl: 60 });
